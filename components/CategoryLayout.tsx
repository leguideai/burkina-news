...
export function CategoryLayout({ categoryCode }: CategoryLayoutProps) {
      ...
      <header className="bg-white border-b border-[#e6dfd5] pt-8 pb-10 px-4 sm:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <nav className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-[#737373] mb-4" aria-label="Breadcrumb">
            ...
            <span>/</span>
            <span className="text-[#141414] font-bold">Rubriques</span>
            <span>/</span>
            <span className="text-[#0b4627] font-bold">{category.nameFr}</span>
          </nav>
            ...
            {otherArticles.length > 0 && (
              <div className="space-y-4 pt-4">
                <h3 className="text-lg font-bold font-serif text-[#141414] pb-2 border-b border-[#e6dfd5]">
                  Toutes les publications · {category.nameFr}
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {otherArticles.map(art => (
                    <ArticleCard key={art.id} article={art} variant="default" />
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-[#faf8f5] border border-[#e6dfd5] p-5">
              ...
              <p className="font-serif text-[#444444] text-xs leading-relaxed mb-4">
                Chaque publication de la rubrique {category.nameFr} s'appuie sur une documentation institutionnelle vérifiable, des visites de terrain et un contrôle méthodologique strict.
              </p>
              <Link href="/fr/methode" className="text-xs font-mono font-bold text-[#0b4627] hover:underline inline-flex items-center gap-1">
                Consulter notre charte de preuve →
              </Link>
            </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center pb-2 border-b border-[#141414]">
                  <h4 className="font-mono text-xs font-bold uppercase tracking-wider text-[#141414]">
                    Projets liés dans le Tracker
                  </h4>
                  <Link href={`/fr/tracker`} className="text-xs font-mono text-[#0b4627] hover:underline">
                    Tout voir →
                  </Link>
                </div>
                ...
                <div className="space-y-3">
                  {indicators.map(ind => (
                    ...
                    >
                      ...
                      <span className="text-[10px] font-mono text-[#737373] mt-1 block">Cible 2030 : {ind.target2030} {ind.unit}</span>
                    </Link>
                  ))}
                </div>
              </div>
